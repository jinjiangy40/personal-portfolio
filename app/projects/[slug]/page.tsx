...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className={`case-page ${project.slug}`}>
    ...
    <section className="case-panel evidence"><div className="section-head compact"><p>03 / VISUAL EVIDENCE</p><h2>项目影像<br />等待补充。</h2></div><div className="evidence-grid"><figure><span>01</span><b>最终成品 / 核心界面</b><small>建议横向主视觉</small></figure><figure><span>02</span><b>使用过程 / 功能演示</b><small>建议截图或短视频</small></figure><figure><span>03</span><b>草图 / 制作过程</b><small>建议 2-4 张过程图</small></figure></div></section>
    <section className="case-panel results"><div><p className="eyebrow">04 / DELIVERED</p><h2>最终交付</h2></div><div>{project.outcome.map((o,i) => <article key={o}><b>0{i+1}</b><p>{o}</p></article>)}</div></section>
    <section className="case-panel reflection"><p className="eyebrow">05 / REFLECTION</p><blockquote>{project.reflection}</blockquote><div><Link className="button primary" href={`/projects/${next.slug}`}>查看下一个案例 ↗</Link><Link className="button" href="/#contact">联系我</Link></div></section>
  </main>;
}
